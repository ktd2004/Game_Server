...
public:
	struct Properties
	{
		...
		NetEventHandler*	pNetwork;					///< PAgent와 연결할 NetEventHandler
...
private:
	ServerApp(const ServerApp& r) : NetEventHandler(0) {}
...
public:
	...
	*/
	...
	/** 
		클라이언트 접속시 호출되는 이벤트 핸들러
		@param spLink 클라이언트 NetLink
	*/
	virtual bool	OnAccepted( NetLinkPtr spLink );
	/** 
		서버 접속시 호출되는 이벤트 핸들러
		@param spLink 서버 NetLink
	*/
	virtual bool	OnConnected( NetLinkPtr spLink );
	/** 
		접속 종료시 호출되는 이벤트 핸들러
		@param spLink 접속된 NetLink
	*/
...
protected:
	...
	///< network event
	bool			OnServerAccepted( NetLinkPtr spLink );
	bool			OnServerConnected( NetLinkPtr spLink );
	bool			OnServerClosed( NetLinkPtr spLink );
};
