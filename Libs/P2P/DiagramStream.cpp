...
DiagramStream::DiagramStream( const DiagramStream &r )
{
...
bool DiagramStream::Open( const std::string& pszIP, unsigned short iPort, unsigned int iMaxBufferSize )
{
	if( m_hSocket == INVALID_SOCKET )
	{
		...
		if ( iResult == SOCKET_ERROR || iResult == -1 )
		{
			...
			prn_err( "bind socket error(%d)", dwErrorCode );
...
bool DiagramStream::IsOpen()
{
	return m_hSocket != INVALID_SOCKET ? true : false;
}

bool DiagramStream::GetSockInfo( std::string& sIP, unsigned short& iPort )
{
	struct sockaddr_in addr;
...
int DiagramStream::Send( const char* pszBuf, unsigned int iLen, SOCKADDR_IN* to )
{
	FD_ZERO( &m_wFdSet );
	FD_SET( m_hSocket, &m_wFdSet);

	int iResult, iPos = 0;
	iResult = select( 0, nullptr, &m_wFdSet, nullptr, &m_Wait );
	if ( iResult == 0 )
	{
		return 0;
	}
...
retry_send:
	...
	if ( iResult == SOCKET_ERROR )
	{
		...
		{
			prn_err( "sendto error(%d)", dwErrorCode );
			return -1;
		}
	}
...
bool DiagramStream::IsReceived( unsigned long iWait )
{
	...
	int iResult;
	iResult = select( 0, &m_rFdSet, nullptr, nullptr, &m_Wait );
	if ( iResult == 0 )
	{
		return false;
	}
...
int DiagramStream::RecvFrom( char* pszBuf, SOCKADDR_IN* from )
{
	...
	if (iResult == SOCKET_ERROR)
	{
		...
		{
			prn_err( "recvfrom error(%d)", dwErrorCode );
			return -1;
		}
