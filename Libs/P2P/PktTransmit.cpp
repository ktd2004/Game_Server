...
PktTransmit::PktTransmit( const PktTransmit &r )
{
...
bool PktTransmit::Open( const std::string& pszIP, unsigned short iPort, unsigned int iMaxBufferSize )
{
	if( m_hSocket == INVALID_SOCKET )
	{
		...
		if ( iResult == SOCKET_ERROR || iResult == -1 )
		{
			...
			verbose( "error: Open() socket Error = %d\n", dwErrorCode );
...
bool PktTransmit::IsOpen()
{
	return m_hSocket != INVALID_SOCKET ? true : false;
}

bool PktTransmit::GetSockInfo( std::string& sIP, unsigned short& iPort )
{
	struct sockaddr_in addr;
...
int PktTransmit::Send( const char* pszBuf, unsigned int iLen, SOCKADDR_IN* to )
{
	FD_ZERO( &m_wFdSet );
...
bool PktTransmit::IsReceived( unsigned long iWait )
{
...
int PktTransmit::RecvFrom( char* pszBuf, SOCKADDR_IN* from )
{
