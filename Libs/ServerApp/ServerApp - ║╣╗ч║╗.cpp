...
, m_iExitCode(0)
{
	...
	SetAcceptedFunc	( this, &ServerApp::OnServerAccepted );
	SetConnectedFunc( this, &ServerApp::OnServerConnected );
	SetClosedFunc	( this, &ServerApp::OnServerClosed );
}
...
bool ServerApp::Initialize(void)
{
	...
	NotifyTask( m_AppProp.iNotifyTaskSchedulerID );

	//////////////////////////////////////////////////////////////////////////
	// proactor 실행
	if ( !Start(0, m_AppProp.iNetworkThreadsCount) )
	{
		prn_err( "%s", errmsg );
		return false;
	}
	...
	agentCfg.pNet = m_AppProp.pNetwork;
...
bool ServerApp::OnAccepted( NetLinkPtr spLink )
{
	return false;
}

bool ServerApp::OnConnected( NetLinkPtr spLink )
{
	return false;
}

bool ServerApp::OnClosed( NetLinkPtr spLink )
{
	return false;
}

bool ServerApp::OnServerAccepted( NetLinkPtr spLink )
{
	OnAccepted( spLink );
	return true;
}

bool ServerApp::OnServerConnected( NetLinkPtr spLink )
{
	if ( !m_PAgent.OnConnected(spLink) )
	{
		OnConnected(spLink);
	}
	return true;
...
bool ServerApp::OnServerClosed( NetLinkPtr spLink )
{
	if ( !m_PAgent.OnClosed(spLink) )
	{
		OnClosed( spLink );
	}
	return true;
...
void ServerApp::SetProperty( ServerApp::Properties& Prop )
{
	...
	m_AppProp.pNetwork = Prop.pNetwork;
