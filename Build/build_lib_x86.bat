...
:: Build Log
SET BUILDLOG=Build_lib_x86_%LOGDATE%_%LOGTIME%.log
...
:: File Check
IF NOT EXIST %LIBLIST% (
	ECHO Error : Not found file - %LIBLIST%
	GOTO QUIT
)
...
:HELP
ECHO Usage : build_lib_x86.bat [-option [-composition]]
...
FOR /f %%P IN (%LIBLIST%) DO (
	ECHO %OPT% - %%P
	@CALL %MSDEV% ..\RPC.NET.ver.0.1.sln %OPT% "%COMPOSITION%|WIN32" /Project %%P /ProjectConfig "%COMPOSITION%|WIN32" /Out %BUILDLOG%
)
