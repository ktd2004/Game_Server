...
:: Build Log
SET BUILDLOG=Build_lib_x64_%LOGDATE%_%LOGTIME%.log
...
:HELP
ECHO Usage : build_lib_x64.bat [-option [-composition]]
...
FOR /f %%P IN (%LIBLIST%) DO (
	ECHO %OPT% - %%P
	@CALL %MSDEV% ..\RPC.NET.ver.0.1.sln %OPT% "%COMPOSITION%|x64" /Project %%P /ProjectConfig "%COMPOSITION%|x64" /Out %BUILDLOG%
)
